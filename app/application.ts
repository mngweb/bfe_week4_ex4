...
class Sprite extends Obstacle {
    ...
    checkCollision(anotherRect : Rect) : Side {
        ...
        if(this.sprite.classList.contains("col-7") && this.sprite.classList.contains("row-7")){
            console.log(this);
        }
...
class Brick extends Sprite {
    ...
    addScore(actualScore: number): number{
        return actualScore += 20;
    }
}
...
class Game {
    ...
    constructor(ballElement : HTMLElement, paddle: HTMLElement, bricks: HTMLCollection, boardElement : HTMLElement, public livesLabel : HTMLElement,
     ...
     createBricks(){             
        ...
        for (let i = 0; i < this.bricksHTML.length; i++) {

            var brickHTML: HTMLElement = <HTMLElement>this.bricksHTML[i],
                brick: Brick;

            brickHTML.style.display = "block"; 
            brickHTML.classList.remove("hard-brick");
            brickHTML.classList.remove("got-hit");

            var binaryRandom = Math.floor(Math.random() * 2);

            if(binaryRandom == 1){  
                brick = new HardBrick(brickHTML)
                brick.sprite.classList.add("hard-brick");
                this.bricks.push(brick);
            }
            else if(binaryRandom == 0){
                brick = new Brick(brickHTML);
                this.bricks.push(brick);
            }
        }       
       ...
       setInterval(() => {
            ...
            for (let brick of this.bricks) {
                ...
                if (wasHit){
                    if(!(--brick.hitsLeft)){ 
                        brick.hide();
                        this.score = brick.addScore(this.score);
                    }
                    else{
                        brick.sprite.classList.add("got-hit");
                    }
                    this.scoreLabel.innerText = '' + this.score;
                    break;
                }
